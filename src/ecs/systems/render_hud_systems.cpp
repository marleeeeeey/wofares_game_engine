...
void RenderHUDSystem(entt::registry& registry, SDL_Renderer* renderer)
{
    ...
    auto cameraScale = gameState.renderingOptions.cameraScale;
...
void DrawGridSystem(SDL_Renderer* renderer, const GameState& gameState)
{
    ...
    int windowWidth = static_cast<int>(gameState.renderingOptions.windowSize.x);
    int windowHeight = static_cast<int>(gameState.renderingOptions.windowSize.y);

    auto& cameraCenter = gameState.renderingOptions.cameraCenter;

    // Calculate the start and end points for drawing the grid
    int startX = static_cast<int>(cameraCenter.x - windowWidth / 2 / gameState.renderingOptions.cameraScale);
    int startY = static_cast<int>(cameraCenter.y - windowHeight / 2 / gameState.renderingOptions.cameraScale);
    int endX = startX + windowWidth / gameState.renderingOptions.cameraScale;
    int endY = startY + windowHeight / gameState.renderingOptions.cameraScale;
    ...
    {
        int screenX = static_cast<int>((x - cameraCenter.x) * gameState.renderingOptions.cameraScale + windowWidth / 2);
    ...
    {
        int screenY =
            static_cast<int>((y - cameraCenter.y) * gameState.renderingOptions.cameraScale + windowHeight / 2);
        SDL_RenderDrawLine(renderer, 0, screenY, windowWidth, screenY);
    }

    // Draw the center of screen point
    DrawCross(renderer, gameState.renderingOptions.windowSize / 2.0f, 20, screenCenterColor);
}
