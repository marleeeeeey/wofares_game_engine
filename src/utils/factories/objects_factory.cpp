...
entt::entity ObjectsFactory::SpawnFragmentAfterExplosion(const glm::vec2& posWorld)
{
    ...
    registry.emplace<PhysicsComponent>(
        entity, box2dBodyCreator.CreatePhysicsBody(entity, posWorld, fragmentAnimation.hitboxWorld));
    ...
    const std::string& animationName, const std::string& tagName, ResourceManager::TagProps tagProps)
{
    ...
    else
    {
        ...
        animationInfo.hitboxWorld = playerSdlSize - glm::vec2{gap, gap};
    }
...
entt::entity ObjectsFactory::SpawnBullet(entt::entity playerEntity, float initialBulletSpeed, AnglePolicy anglePolicy)
{
    ...
    const auto& playerSize = animationInfo.animation.frames.front().renderingInfo.sizeWorld;
